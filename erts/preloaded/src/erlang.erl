...
> cases, use [`binary_to_term/2`](`binary_to_term/2`) with the `safe`
> option.
...
- **`safe`** - Use this option when receiving binaries from an untrusted source.
  ...
  ```erlang
  1> Bin = <<131,119,5,"tjena">>.
  2> binary_to_term(Bin, [safe]).
  ** exception error: bad argument
       in function  binary_to_term/2
          called as binary_to_term(<<131,119,5,116,106,101,110,97>>,[safe])
...
> need to check some proprty of a fun should use `fun_info/2`.
