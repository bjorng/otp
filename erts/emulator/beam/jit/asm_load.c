...
int beam_load_prepare_emit(LoaderState *stp) {
    ...
    if (stp->beam.lines.flags & BEAMFILE_FORCE_LINE_COUNTERS) {
        ...
        /* A coverage mode is enabled, but there are no
         * executable_line instructions in this module. To ensure that
         * get_function_coverage/1 will always work, we will force the
         * coverage mode to `function`. */
        hdr->coverage_mode = ERTS_COV_FUNCTION_COVERAGE;
    } else {
...
#    endif
    ...
    case op_executable_line_I: {
        ...
        if (stp->load_hdr->coverage_mode == ERTS_COV_LINE_COVERAGE) {
            coverage_size = sizeof(byte);
    ...
    case op_i_test_yield:
        if (stp->load_hdr->coverage_mode == ERTS_COV_FUNCTION_COVERAGE) {
            ASSERT(stp->function_number != 0);
            beamasm_emit_coverage(stp->ba,
                                  stp->coverage,
