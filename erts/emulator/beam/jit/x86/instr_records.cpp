...
void BeamModuleAssembler::emit_is_native_record(const ArgLabel &Fail,
    ...
    mov_preserve_cache(
            ...
            emit_boxed_val(boxed_ptr,
                           ...
                           sizeof(Uint64)));
    cmp_arg(emit_boxed_val(RET, offsetof(ErtsRecordDefinition, module)),
            Module);
    preserve_cache([&]() {
        a.jne(resolve_beam_label(Fail));
    });
    cmp_arg(emit_boxed_val(RET, offsetof(ErtsRecordDefinition, name)), Name);
    preserve_cache([&]() {
        a.jne(resolve_beam_label(Fail));
    });
}
...
void BeamModuleAssembler::emit_is_record_accessible(const ArgLabel &Fail,
    ...
    mov_preserve_cache(
            ...
            emit_boxed_val(boxed_ptr,
                           offsetof(ErtsRecordInstance, record_definition)));
    cmp_arg(emit_boxed_val(RET, offsetof(ErtsRecordDefinition, is_exported)),
            ArgAtom(am_true));
