...
void BeamModuleAssembler::emit_i_mul_add(const ArgLabel &Fail,
    ...
    if (always_small(Src1) && always_small(Src2) && always_small(Src4)) {
         ...
         * code fragment to performt the operation. */
    ...
    if (is_product_small && is_sum_small) {
        ...
        if (left_shift > 0) {
            comment("optimized multiplication by replacing with left "
                    "shift");
            a.add(dst.reg, increment_reg, TMP3, a64::lsl(left_shift));
        } else if (Src2.isSmall() && Support::is_power_of_2(factor - 1)) {
            comment("optimized multiplication by replacing with left "
                    "shift and addition");
            left_shift = Support::ctz<Eterm>(factor - 1);
            if (is_increment_zero) {
                a.add(dst.reg, src1.reg, TMP3, a64::lsl(left_shift));
            } else {
                a.add(dst.reg, increment_reg, TMP3, a64::lsl(left_shift));
            ...
            a.madd(dst.reg, TMP3, TMP4, increment_reg);
        }
        ...
        if (is_increment_zero) {
            a.add(dst.reg, dst.reg, imm(_TAG_IMMED1_SMALL));
        } else {
            a64::Gp sign_reg;
