...
can be changed at any time without causing any compatibility issues.
...
* The implementation of each generic instruction.  **beam\_makeops**
will generate a specific implementation for each specific instruction
corresponding to the generic instruction.

Generic instructions have typed operands. For example, here are a few
examples of operands for `move/2` (using the same syntax that the
compiler uses):
...
Corresponding to each generic instruction, there is a whole family of
...
Let's look at the `move/2` instruction.
...
We will look at the code for each specific instruction in turn,
starting with `move_cx` because it is somewhat simpler than the other
two.  To make the code easier to understand, let's first look at the
memory layout for the instruction `{move,{atom,id},{x,5}}`:
...
The memory layout is for a 64-bit archictecture.  `I` in the BEAM
virtual machine is the instruction pointer.  When BEAM executes an
instruction, `I` points to the first word of the instruction.
...
the byte offset to the X register, that is the 5 has been multiplied
by the word size size 8.

In the new word, the tagged atom 'id' is stored.
...
pointer to an `Eterm` pointer and dereference it.  On the right side
of the `=`, `I[1]` will fetch the tagged atom 'id'.
...
`move_xy` is almost identical to `move_xx`.  The only difference is the
use of the `yb()` macro instead of `xb()` for the destination register:
...
Using the following rule we can combine two such `move` instructions
to a `move2` instruction:

    move X1=x Y1=y | move X2=x Y2=y => move2 X1 Y1 X2 Y2

The left side of the arrow (`=>`) is a pattern.  If the pattern matches,
the matching instructions will be replaced with right side.  Variables
in a pattern must start with an uppercase letter just as in Erlang.
The variable may be followed `=` and one or more type letters to constrain
the match to just those types.  The variables that are bound on the left
side can be used on the right side.
...
Because of that, we can define the rule for the `move3/6` instruction
as follows:
...
Here is a quick overview of how instructions are loaded.

* The loader reads and decodes one instruction at a time from the BEAM
code and creates a generic instructions.  Because many transformation
rules attempts to transform multiple instructions, the loader will
...
in the code are for the module being loaded.

* The loader translates each operand to a machine word and stores it
in the code area.  The operand type given in the specific guides the
translation.  For example, if the type is `e`, the value of the
operand is an index into an arry of external functions and will be
translated to a pointer to the export entry for the function to call.
If the type is `x`, the number of the X register will be multiplied
by the word size to produce a byte offset.

* The loader runs the packing engine to pack multiple operands into a
single word.  The packing engine is controlled by a small program that
is a string where each character is an instruction.  For example, the
code to pack the operands for `move_xy` is `"22#"` (on a 64-bit
machine).  That program will pack the byte offsets for both registers
into the same word as the pointer to C code.
...
A long line can be broken into shorter lines by a placing `\` before
the newline.

### Defining the format number ###

This line in `genop.tab` should not be changed:

    BEAM_FORMAT_NUMBER=0

It defines the version of the instruction set.  Theoretically,
...
TODO: %if %unless %else %endif

TODO: ARCH_64 NO_FPE_SIGNALS
...
obsoleted.  The compiler is not allowed go generate BEAM files
containing obsolted instructions and the loader will refuse to load
BEAM files that use obsoleted instructions.
...
system and can be changed any time without compatibility issues.
...
* Explicitly.  This is necessary in rare circumstances when a generic
instruction does not have any corresponding specific instruction.
...
Each generic instruction has an opcode.  For an external generic
instruction, it must be explicitly given `genop.tab`.  Internal generic
instructions are automatically numbered by **beam\_makeops**.

The identity of a generic instruction is its name combined with its arity.
That means that is allowed to define two distinct generic instructions
having the same name but with different arities.  For example:
...
* `u` - An untagged integer that fits in a machine word.  It is use for many
...
* `l` - A floating point register number.

* `i` - A tagged literal integer.

* `a` - A tagged literal atom.

* `n` - NIL (`[]`, the empty list).

* `q` - Literals that don't fit in a word, that is, one of literal
list, tuple, binary, bignum, or map.
...
followed by the types for each operand.  For example:

     move x y

Internally, for example in the generated code and in the output from
the BEAM disassembler, the insruction `move x y` will be called `move_xy`.

The name for a specific instruction an identifier starting with a
...
* `l` - A floating point register number.  (Can be packed with other
operands.)

* `i` - A tagged literal integer (a SMALL that will fit in one word).

* `a` - A tagged atom.

* `n` - NIL or the empty list.  (Will not be stored in the loaded code.)

* `q` - A tagged CONS or BOXED pointer.  That is, a literal list, tuple,
binary, bignum, or map.

* `f` - A failure label (non-zero).  The target for a branch
or call instruction.  References a label; will be replaced the address of the
label when loading is finished.
...
The types that follow are generally more expensive in terms of runtime
than the types described earlier.  However, those operand types are
needed to avoid a combinatorial explosion in the number of specific
instructions.

* `s` - Tagged source.  May be an X or Y register, or any literal term.
The tag will be tested at runtime to retrieve the value from an X register,
a Y register, or simply use the tagged Erlang term.
...
at runtime to set up a pointer to the destination register at the
beginning of the instruction.  If the instrution performs a garbarge
collection, it must use the `$REFRESH_GEN_DEST()` macro to refresh the
pointer (there are more details about that in a later section of this document).
...
packed with other operands in a word.
...
The loader will translate generic instructions to specific
instructions.  In doing so, it will choose the most specific
instruction that will fit the constraints.  Consider the following
two instructions:
...
because `move n x` is more specific.
...
different words.  Thus, at runtime the `allocate_heap` must read both
words from memory even though it will not always use "Live register".
...
A rule is recognized by its arrow, `=>`.  To the left of the arrow is
one or more instruction patterns, separated by `|`.  To the right of
the arrow is zero or more instructions, separated by `|`.  If the
instructions from the BEAM code matches the instruction patterns on
the left side, they will be replaced with instructions on the right
side (or removed if there are no instructions on the right).
...
actual rule a long time when the compiler would occasionally produce
...
If we only want to match operands that are of a certain type, we can
...
The second operand pattern, `an`, will match if the second operand is either
an atom or NIL (the empty list).  In case of a match, the `is_integer/2` instruction
will be replaced with an unconditional `jump/1` insruction.

An operand pattern can both bind a variable and constrain the type at the same
time by following the variable with a `=` and the constraint.  For example:
...
known to contain an index to the import table chunk in the BEAM file.  If applied
to other `u` operands, it will at best return a nonsense result.
...
a nicer error message than the default one would be for a missing guard BIF.
...
guard function on the left side.  If the guard function returns a
non-zero value, the matching of the rule will continue, otherwise the
match will fail.  For example:
...
An operand can also be a type letter to construct a value of that type.
...
X register 1023.  Then `is_number/2` will test whether X register 1023
is a number.
...
* `l` - A floating point register number.  The default value is 0.

* `i` - A tagged literal integer.  The default value is 0.

* `a` - A tagged atom.  The default value is the empty atom (`am_Empty`).
...
generation function on the right side.  For example:
...
instruction will have to fetch the index from a register and then
untag the integer.
...
this macro implements `deallocate_return/1`:
...
To avoiding repeating code, `deallocate_return/1` can be invoked
from `move_deallocate_return/2`:
...
`$deallocate_return($Deallocate)` will insert the definition of
`deallocate_return/1` into the body of `move_deallocate_return/2`
wrapped in a `do`/`while` block, similar to this:
...
(The `do`/`while` wrapper will be omitted when **beam\_makeops**
can clearly see that it is not needed.)
...
To see what they do, let's first look at the default code generation.
Here is the code generated for `move_cx`:
...
operand is an atom) or branch to the failure label.  We don't want
any prefetch.
...
Let's look at the `-no_next` directive.  Here is the `jump/1`
instruction:
...
The file `macros.tab` contains many useful macros.  A few of them are
described here.
...
There are several built-in macros.
...
syntax as C).  If `Expr` evaluates to 0, the $IF() expression will be
...
When a specific instruction has a `d` operand, the operand will
be fetched in the beginning of the instruction and a pointer
will be set up to the X or Y register that `d` operand references.

If there is a garbage collection before the result is stored,
the stack will move and if the `d` operand refered to an Y
register, the pointer will no longer be valid.
...
**Problem**: For frequently executed instructions for performance we
want to use "fast" operands types such as `x` and `y`, as opposed to
`s` or `S`.  To avoid an explosion in code size, we want to share the
most of the implementation between the instructions.  Here are the
specific operations for `i_increment/5`:

   i_increment r W t d
   i_increment x W t d
   i_increment y W t d

We could implement it like this if we want the highest performance:
...
To avoid three almost identical copies of the code, we use only one
specific instruction:

    i_increment S W t d

That reduces the code size, but is slower because `S` means that
there will be extra code to test whether the operand refers to a X
register or an Y register.
...
we can the value of the register in:
...
belongs to.  Note that it does not need to have the same same
...
Next we define the fragment that we pick up the value from the
...
**beam\_makeops** howe they should be connected:
...
The operand and/or operands that is different must be at
...
We would have to change the order of the operands:
...
other two only have two segments.  Here are the fragments:
