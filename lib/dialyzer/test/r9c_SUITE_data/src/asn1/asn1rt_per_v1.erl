...
fixoptionals2(OptList,OptLength,Val) when is_tuple(Val) ->
...
%% BinBits = binary(),
%% RestBytes = tuple()
getbits_as_binary(Num,Bytes) when is_list(Bytes) ->
...
encode_integer(C,V,_) when is_integer(V) ->
    encode_integer(C,V);
...
encode_length(SingleValue,_) when is_integer(SingleValue) ->
    [].
...
encode_bit_string(C, BitListValue, NamedBitList) when is_list(BitListValue) ->
	...
	0 -> % fixed length
        ...
        {Lb,Ub} when is_integer(Lb),is_integer(Ub),BListLen<Lb ->
	...
	no ->
        ...
        Sc={{Lb,Ub},_} when is_integer(Lb),is_integer(Ub),BListLen<Lb ->
	    %% padding due to OTP-4353
...
decode_compact_bit_string(Buffer, C, NamedNumberList) ->
	...
	0 -> % fixed length
        ...
        V when is_integer(V) -> %fixed length > 16 bits
	    Bytes2 = align(Buffer),
