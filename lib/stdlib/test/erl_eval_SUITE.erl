...
all() -> 
    [guard_1, guard_2, match_pattern, string_plusplus,
     ...
     eep49, binary_and_map_aliases, eep58, zlc, zbc, zmc].
...
zbc(Config) when is_list(Config) ->
    ...
    check(fun() -> 
        ...
        <<>>),
    check(fun() -> 
         << <<X:32>> || <<X:bad>> <= <<1,2,3>> && <<Y>> <= <<1,2,3>> >> end,
        "begin << <<X:32, Y:32>> || <<X:bad>> <= <<1,2,3>> && <<Y>> <= <<1,2,3>> >> end.",
        <<>>),
