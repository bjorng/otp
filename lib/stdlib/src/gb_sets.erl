...
```erlang
> gb_sets:is_empty(gb_sets:new()).
true
> gb_sets:is_empty(gb_sets:singleton(1)).
false
```
""".
...
```erlang
> gb_sets:size(gb_sets:new()).
0
> gb_sets:size(gb_sets:from_list([4,5,6])).
3
```
""".
...
```erlang
> Empty = gb_sets:new().
> S = gb_sets:from_list([a,b]).
> gb_sets:is_equal(S, S)
true
> gb_sets:is_equal(S, Empty)
false
```
""".
-doc(#{since => <<"OTP 27.0">>}).
...
> gb_sets:is_member(42, S).
false
> gb_sets:is_member(b, S).
true
```
""".
...
```erlang
> S0 = gb_sets:new().
> S1 = gb_sets:insert(7, S0).
> gb_sets:to_list(S1).
[7]
> S2 = gb_sets:insert(42, S1).
> gb_sets:to_list(S2).
[7,42]
```
""".
...
```erlang
> S0 = gb_sets:from_ordset(lists:seq(1, 100)).
> Delete = fun(E, Set) -> gb_sets:delete(E, Set) end.
> S1 = lists:foldl(Delete, S0, lists:seq(1, 50)).
> gb_sets:size(S1).
50
...
```erlang
> S0 = gb_sets:new().
> S1 = gb_sets:add_element(7, S0).
> gb_sets:to_list(S1).
[7]
> S2 = gb_sets:add_element(42, S1).
> S2 = gb_sets:add_element(42, S1).
> gb_sets:to_list(S2).
[7,42]
```
...
```erlang
> Unordered = [x,y,a,x,y,b,b,z]
> gb_sets:to_list(gb_sets:from_list(Unordered)).
[a,b,x,y,z]
```
""".
...
```erlang
> Ordset = [1,2,3].
> gb_sets:to_list(gb_sets:from_ordset(Ordset)).
[1,2,3]
```
""".
...
```erlang
> S = gb_sets:from_list([a,b]).
> gb_sets:to_list(gb_sets:delete_any(b, S)).
[a]
> S = gb_sets:delete_any(x, S).
> gb_sets:to_list(S).
[a,b]
```
""".
...
```erlang
> S = gb_sets:from_list([a,b]).
> gb_sets:to_list(gb_sets:delete(b, S)).
[a]
```
""".
...
```erlang
> gb_sets:to_list(gb_sets:from_list([4,3,5,1,2])).
[1,2,3,4,5]
```
""".
...
```erlang
> S = gb_sets:from_ordset([1,2,3,4,5]).
> Iter0 = gb_sets:iterator(S, ordered).
> element(1, gb_sets:next(Iter0)).
1
> Iter1 = gb_sets:iterator(S, reversed).
> element(1, gb_sets:next(Iter1)).
5
```
""".
-doc(#{since => <<"OTP 27.0">>}).
...
```erlang
> S = gb_sets:from_ordset([10,20,30,40,50]).
> Iter = gb_sets:iterator_from(17, S).
> element(1, gb_sets:next(Iter)).
20
```
""".
-doc(#{since => <<"OTP 18.0">>}).
...
```erlang
> S = gb_sets:from_ordset([10,20,30,40,50]).
> Iter = gb_sets:iterator_from(17, S, reversed).
> element(1, gb_sets:next(Iter)).
10
```
""".
...
```erlang
> S = gb_sets:from_ordset([1,2,3,4,5]).
> Iter0 = gb_sets:iterator(S).
> {Element0, Iter1} = gb_sets:next(Iter0).
> Element0.
1
> {Element1, Iter2} = gb_sets:next(Iter1).
> Element1.
2
```
""".
...
```erlang
> S0 = gb_sets:from_list([a,b,c,d]).
> S1 = gb_sets:from_list([c,d,e,f]).
> Union = gb_sets:union(S0, S1).
> gb_sets:to_list(Union).
[a,b,c,d,e,f]
```
""".
...
```erlang
> S0 = gb_sets:from_list([a,b,c,d]).
> S1 = gb_sets:from_list([d,e,f]).
> S2 = gb_sets:from_list([q,r])
> Sets = [S0, S1, S2].
> Union = gb_sets:union(Sets).
> gb_sets:to_list(Union).
[a,b,c,d,e,f,q,r]
```
""".
...
> S1 = gb_sets:from_list([c,d,e,f]).
> S2 = gb_sets:from_list([q,r]).
> gb_sets:to_list(gb_sets:intersection(S0, S1)).
[c,d]
> gb_sets:to_list(gb_sets:intersection(S1, S2)).
[]
```
""".
...
> S2 = gb_sets:from_list([q,r])
> Sets = [S0, S1, S2].
> gb_sets:to_list(gb_sets:intersection([S0, S1, S2])).
[]
> gb_sets:to_list(gb_sets:intersection([S0, S1])).
[d]
```
...
> S1 = gb_sets:from_list([d,e,f]).
> S2 = gb_sets:from_list([q,r])
> gb_sets:is_disjoint(S0, S1).
false
> gb_sets:is_disjoint(S1, S2).
true
```
""".
...
> S0 = gb_sets:from_list([a,b,c,d]).
> S1 = gb_sets:from_list([c,d,e,f]).
> gb_sets:to_list(gb_sets:subtract(S0, S1)).
[a,b]
> gb_sets:to_list(gb_sets:subtract(S1, S0)).
[e,f]
```
""".
...
> gb_sets:is_subset(S1, S0).
true
> gb_sets:is_subset(S0, S1).
false
> gb_sets:is_subset(S0, S0).
true
```
""".
...
```erlang
> gb_sets:is_set(gb_sets:new()).
true
> gb_sets:is_set(gb_sets:singleton(42)).
true
> gb_sets:is_set(0).
false
```
...
```erlang
> S = gb_sets:from_list([1,2,3,4,5,6,7]).
> IsEven = fun(N) -> N rem 2 =:= 0 end.
> Filtered = gb_sets:filter(IsEven, S).
...
> S = gb_sets:from_list([1,2,3,4,5,6,7]).
> F = fun(N) -> N div 2 end.
> Mapped = gb_sets:map(F, S).
> gb_sets:to_list(Mapped).
[0,1,2,3]
```
""".
-doc(#{since => <<"OTP 27.0">>}).
...
```erlang
> S = gb_sets:from_list([2,4,5,6,8,9])
> F = fun(X) ->
          case X rem 2 of
              0 -> {true, X div 2};
              1 -> false
...
```erlang
> S = gb_sets:from_list([1,2,3,4]).
> Plus = fun erlang:'+'/2.
> gb_sets:fold(Plus, 0, S).
10
```
""".
