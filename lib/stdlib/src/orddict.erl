...
3> D2 = orddict:append(files, f1, D1).
4> D3 = orddict:append(files, f2, D2).
5> D4 = orddict:append(files, f3, D3).
6> orddict:fetch(files, D4).
[f1,f2,f3]
```
...
3> S2 = [{files,f2} | S1].
4> S3 = [{files,f3} | S2].
[{files,f3},{files,f2},{files,f1}]
5> S4 = sofs:relation(S3).
6> S5 = sofs:relation_to_family(S4).
7> OrdDict = sofs:to_external(S5).
[{files,[f1,f2,f3]}]
8> orddict:fetch(files, OrdDict).
[f1,f2,f3]
```
