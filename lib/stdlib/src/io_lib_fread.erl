...
-define(is_between(I, Lb, Ub),
        is_integer(I) andalso (Lb) =< (I) andalso (I) =< (Ub)).

-define(is_valid_base(B), ?is_between(B, 2, 1+$Z-$A+10)).
...
fread_field([C|Format], Sup, Unic) when ?is_between(C, $0, $9) ->
...
fread_field([C|Format], F, Sup, Unic) when ?is_between(C, $0, $9) ->
...
fread_chars(N, [C|Line], false, Cs) when ?is_between(C, 0, 255) ->
...
fread_digits([C|Line], N, Cs) when ?is_between(C, $0, $9) ->
    fread_digits(Line, N+1, [C|Cs]);
fread_digits(Line, N, Cs) -> {Line,N,Cs}.

fread_digits([C|Line], N, Base, Cs) when ?is_between(C, $0, $9) ->
    fread_digits(Line, N+1, Base, [C|Cs]);
fread_digits([C|Line], N, Base, Cs) when ?is_between(C, $A, Base+($A-10-1)) ->
    fread_digits(Line, N+1, Base, [C|Cs]);
fread_digits([C|Line], N, Base, Cs) when ?is_between(C, $a, Base+($a-10-1)) ->
    fread_digits(Line, N+1, Base, [C|Cs]);
