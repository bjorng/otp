...
```erlang
> sets:to_list(sets:new([{version, 1}])).
[]
> sets:new() =:= sets:new([{version, 2}]).
true
```
""".
-doc(#{since => <<"OTP 24.0">>}).
...
```erlang
> S = sets:from_list([a,b,c], [{version, 1}]).
> lists:sort(sets:to_list(S)).
[a,b,c]
```
""".
...
```erlang
> sets:is_set(sets:new()).
true
> sets:is_set(sets:new([{version,1}])).
true
> sets:is_set(42).
false
```
...
> sets:size(sets:new()).
0
> sets:size(sets:from_list([4,5,6])).
3
```
""".
...
```erlang
> sets:is_empty(sets:new()).
true
> sets:is_empty(sets:from_list([1])).
false
```
""".
-doc(#{since => <<"OTP 21.0">>}).
...
```erlang
> Empty = sets:new().
> S = sets:from_list([a,b]).
> sets:is_equal(S, S)
true
> sets:is_equal(S, Empty)
false
> OldSet = sets:from_list([a,b], [{version,1}]).
> sets:is_equal(S, OldSet).
true
> S =:= OldSet.
false
```
...
```erlang
> S0 = sets:new().
> S1 = sets:add_element(7, S0).
> sets:to_list(S1).
[7]
> S2 = sets:add_element(42, S1).
...
```erlang
> S = sets:from_list([a,b]).
> sets:to_list(sets:del_element(b, S)).
[a]
> S = sets:del_element(x, S).
> lists:sort(sets:to_list(S)).
[a,b]
```
""".
...
```erlang
> S0 = sets:from_list([a,b,c,d]).
> S1 = sets:from_list([c,d,e,f]).
> Union = sets:union(S0, S1).
> lists:sort(sets:to_list(Union)).
[a,b,c,d,e,f]
```
""".
...
```erlang
> S0 = sets:from_list([a,b,c,d]).
> S1 = sets:from_list([d,e,f]).
> S2 = sets:from_list([q,r])
> Sets = [S0, S1, S2].
> Union = sets:union(Sets).
> lists:sort(sets:to_list(Union)).
[a,b,c,d,e,f,q,r]
```
""".
...
> S1 = sets:from_list([c,d,e,f]).
> S2 = sets:from_list([q,r]).
> Intersection = sets:intersection(S0, S1).
> lists:sort(sets:to_list(Intersection)).
[c,d]
...
> S2 = sets:from_list([q,r])
> Sets = [S0, S1, S2].
> sets:to_list(sets:intersection([S0, S1, S2])).
[]
> sets:to_list(sets:intersection([S0, S1])).
[d]
```
""".
...
> S1 = sets:from_list([d,e,f]).
> S2 = sets:from_list([q,r])
> sets:is_disjoint(S0, S1).
false
> sets:is_disjoint(S1, S2).
true
```
""".
...
> S0 = sets:from_list([a,b,c,d]).
> S1 = sets:from_list([c,d,e,f]).
> lists:sort(sets:to_list(sets:subtract(S0, S1))).
[a,b]
> lists:sort(sets:to_list(sets:subtract(S1, S0))).
[e,f]
```
""".
...
> sets:is_subset(S1, S0).
true
> sets:is_subset(S0, S1).
false
> sets:is_subset(S0, S0).
true
```
""".
...
```erlang
> S = sets:from_list([1,2,3,4,5,6,7]).
> IsEven = fun(N) -> N rem 2 =:= 0 end.
> Filtered = sets:filter(IsEven, S).
> lists:sort(sets:to_list(Filtered)).
[2,4,6]
```
""".
...
> F = fun(N) -> N div 2 end.
> Mapped = sets:map(F, S).
> lists:sort(sets:to_list(Mapped)).
[0,1,2,3]
```
""".
-doc(#{since => <<"OTP 27.0">>}).
