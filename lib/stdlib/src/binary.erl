...
-doc """
Converts a positive integer into the smallest possible unsigned binary
representation, using either big-endian or little-endian format.

## Examples

```erlang
1> binary:encode_unsigned(0).
<<0>>
2> binary:encode_unsigned(255).
<<255>>
3> binary:encode_unsigned(256).
<<1,0>>
4> binary:encode_unsigned(256, little).
<<0,1>>
5> binary:encode_unsigned(11111111).
<<169,138,199>>
6> binary:encode_unsigned(11111111, big).
<<169,138,199>>
7> binary:encode_unsigned(11111111, little).
<<199,138,169>>
```
""".
-doc(#{since => <<"OTP R14B">>}).
