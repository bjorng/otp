...
-doc """
Takes a list of keys and a value and builds a map where all keys have
the same value.
...
-doc """
Returns a complete list of values contained in map `Map`, in arbitrary
order.
...
-doc """
Calls `Fun(Key, Value1)` on each key-value pair of `MapOrIter` in arbitrary order
to update remove associations from `MapOrIter`.
...
Function `fun Fun/3` must return a new accumulator, which is passed to
the next. The function returns the final value of the accumulator. The
