...
get_strict_patterns([{m_generate_strict,Lg,{map_field_exact,_,K0,V0},_E}|Gs], {Line, St0}, Acc) ->
...
replace_vars(#c_cons{hd=H,tl=T}, Vars, St0) ->
    ...
    {#c_cons{hd=H1,tl=T1}, St2};
replace_vars(#c_tuple{es=Es0}, Vars, St0) ->
    {Es1, St1} = replace_list_vars(Es0, Vars, St0),
    {#c_tuple{es=Es1}, St1};
replace_vars(#c_map{arg=V,es=Es0}, Vars, St0) ->
    {Es1, St1} = replace_list_vars(Es0, Vars, St0),
    {V1, St2} = replace_vars(V, Vars, St1),
    {#c_map{arg=V1,es=Es1}, St2};
replace_vars(#c_map_pair{key=K,val=V}, Vars, St0) ->
    {V1, St1} = replace_vars(V, Vars, St0),
    {K1, St2} = replace_vars(K, Vars, St1),
    {#c_map_pair{key=K1,val=V1}, St2};
...
generator(Line, {Generate,Lg,P0,E}, Gs, StrictPats, St0) when Generate =:= generate;
    ...
    {Gen,St5};
generator(Line, {Generate,Lg,P,E}, Gs, _StrictPats, St0) when Generate =:= b_generate;
...
generator(Line, {Generate,Lg,{map_field_exact,_,K0,V0},E}, Gs, StrictPats, St0) when
        Generate =:= m_generate;
    ...
    {NomatchPat, St5} = case {Pat, Generate} of
        {nomatch, _} -> {#c_tuple{es=[NomatchK,NomatchV,IterVar]}, St4};
        {_, m_generate} ->
            {Pat1, St} = replace_vars(Pat, StrictPats, St3),
            case lists:all(fun (V) -> is_integer(V) orelse V =:= '_' end, lit_vars(Pat1)) of
                true -> {#c_tuple{es=[NomatchK,NomatchV,IterVar]}, St4};
                false -> {#c_tuple{es=[cons_hd(Pat1),cons_tl(Pat1),IterVar]}, St}
            end;
        {_, m_generate_strict} ->
            {AccPat, St4}
        end,
