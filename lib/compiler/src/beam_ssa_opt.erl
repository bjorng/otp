...
early_epilogue_passes(Opts) ->
    Ps = [?PASS(ssa_opt_bs_match),
          ?PASS(ssa_opt_type_continue),         %Fix types after ssa_opt_bs_match.
          ?PASS(ssa_opt_type_finish),
          ?PASS(ssa_opt_float),
          ?PASS(ssa_opt_sw),
...
%%%
%%% Fix up binary matching.
%%%

ssa_opt_bs_match({#opt_st{ssa=Linear0,cnt=Count0}=St, FuncDb})
  when is_list(Linear0) ->
    Blocks0 = maps:from_list(Linear0),
    RPO = beam_ssa:rpo(Blocks0),
    Sub = #{},
    {Dom, _} = beam_ssa:dominators(RPO, Blocks0),
    Blocks1 = bjorn(RPO, Dom, Sub, Blocks0),
    {Blocks, Count} = bs_add_block(Blocks1, Count0),
    Linear = beam_ssa:linearize(Blocks),
    {St#opt_st{ssa=Linear,cnt=Count}, FuncDb}.
...
bs_add_block(BlockMap0, Count0) ->
    SSA = beam_ssa:linearize(BlockMap0),

    %% Search from all bs_start_match instructions.
    bs_find_start_match(SSA, BlockMap0, Count0, sets:new()).
