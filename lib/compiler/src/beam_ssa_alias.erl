...
aa_tuple_extraction(Dst, Tuple, Idx, SS1) ->
    case {SS1,Idx} of
        {#{{tuple_element,Tuple}:=all},_} ->
            aa_set_aliased([Dst,Tuple], SS1);
        {#{{tuple_element,Tuple}:=OrdSet0},#b_literal{val=I}} ->
        ...
        {#{{tuple_element,Tuple}:=_OrdSet},_} ->
            %% We have to assume that we have already extracted the
            %% element.
            aa_set_aliased([Dst,Tuple], SS1);
        {_,#b_literal{val=I}} ->
        ...
        {_,_} ->
            %% There are no aliases yet and we don't know which
            %% element we are extracting.
            aa_join(Dst, Tuple, SS1#{{tuple_element,Tuple}=>all})
    end.
