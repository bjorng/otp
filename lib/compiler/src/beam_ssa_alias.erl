...
expand_record_update(#opt_st{ssa=Linear0,cnt=First,anno=Anno0}=OptSt) ->
    {Linear,Cnt} = eur_blocks(Linear0, First),
    Anno = Anno0#{orig_cnt=>First},
    OptSt#opt_st{ssa=Linear,cnt=Cnt,anno=Anno}.

eur_blocks(Linear, First) ->
    eur_blocks(Linear, First, []).

eur_blocks([{Lbl,#b_blk{is=Is0}=Blk}|Rest], First, Acc) ->
    {Is,Next} = eur_is(Is0, First, []),
    eur_blocks(Rest, Next, [{Lbl,Blk#b_blk{is=Is}}|Acc]);
eur_blocks([], Cnt, Acc) ->
    {reverse(Acc),Cnt}.

eur_is([#b_set{op=update_record,
    ...
    {Extracts,ExtraArgs,Next,ArgTypes} =
        eur_args(Updates, First, Src, Size, TupleType, ArgTypes0),
    ...
    eur_is(Rest, Next, [I|Extracts]++Acc);
eur_is([I|Rest], First, Acc) ->
    eur_is(Rest, First, [I|Acc]);
eur_is([], First, Acc) ->
    {reverse(Acc), First}.

eur_args(Updates, First, Src, Size, TupleType, ArgTypes) ->
    eur_args1(Updates, sets:from_list(lists:seq(1, Size), [{version,2}]),
              4, First, Src, TupleType, ArgTypes).

eur_args1([#b_literal{val=Idx},_Val|Updates],
          Remaining, ArgIdx, First, Src, TupleType, ArgTypes) ->
    eur_args1(Updates, sets:del_element(Idx, Remaining), ArgIdx+2,
              First, Src, TupleType, ArgTypes);
eur_args1([], Remaining, ArgIdx, First, Src, TupleType, ArgTypes) ->
    eur_args2(sets:to_list(Remaining), [], [], ArgIdx,
              First, Src, TupleType, ArgTypes).

eur_args2([Idx|Remaining], Extracts, Args0, ArgIdx, First,
    ...
    eur_args2(Remaining, [I|Extracts], Args,
              ArgIdx+2, First+1, Src, TupleType, ArgTypes);
eur_args2([], Extracts, Args, _, First, _, _, ArgTypes) ->
