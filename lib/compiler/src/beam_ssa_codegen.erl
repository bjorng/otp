...
delay_alloc([#cg_alloc{}=AI,
             #cg_set{op=executable_line}=ELI|Is2]) ->
...
%%%
%%% Gather debug information and add as annotations to executable_line
%%% instructions.
%%%
...
%%%
%%% Annotate `executable_line` instructions with all variables that
%%% have been defined and are still available in a BEAM register.
%%%
...
def_regs_is([#cg_set{op=catch_end,dst=Dst,args=[#b_var{}=Tag|_]}=I|Is], Regs, Def0, Acc) ->
    ...
    def_regs_is(Is, Regs, Def, [I|Acc]);
def_regs_is([#cg_set{anno=Anno0,op=executable_line}=I0|Is], Regs, Def, Acc) ->
...
cg_instr(executable_line, [{integer,Index},{literal,Info}], _Dst, #cg_set{anno=Anno}) ->
    {line,Location} = line(Anno),
    [{executable_line,Location,Index,Info}];
