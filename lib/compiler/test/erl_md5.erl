...
md5(Msg0) when is_binary(Msg0) ->
    ...
    Msg = pad(Msg0),
    hash1(Msg, A0, B0, C0, D0).

hash1(<<>>, A, B, C, D) ->
    <<A:32/little, B:32/little, C:32/little, D:32/little>>;
hash1(<<Msg0/binary>>, A0, B0, C0, D0) ->
    {A,B,C,D} = hash_part(Msg0, A0, B0, C0, D0, 0),
    <<_:512, Msg/binary>> = Msg0,
    hash1(Msg,
          ?CLAMP(A0+A),
          ?CLAMP(B0+B),
          ?CLAMP(C0+C),
...
hash_part(<<Msg/binary>> = Msg0, A, B, C, D, I) when I < 64 ->
    if
        I =< 15 ->
    ...
    F = ?CLAMP(F0 + A + M + ?CONST(I)),
    FRot = (F bsl Shift) bor (F bsr (32-Shift)),
    BUpd = ?CLAMP(B + FRot),
    %% A <= D,  B <= B + F_Rotated, D <= C, C <= B,
    hash_part(Msg0, D, BUpd, B, C, I+1);
hash_part(_, A, B, C, D, _) ->
    {A,B,C,D}.

pad(Msg) ->
    Size = bit_size(Msg),
    SizeM = Size rem ((1 bsl 64)-1),
    ...
    R = <<Msg/bits, 1:1, 0:Zeros, SizeM:64/little>>,
    true = is_binary(R),
    R.
