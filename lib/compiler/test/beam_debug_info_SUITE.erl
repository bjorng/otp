...
smoke(_Config) ->
    ...
    S = ~"""
         ...
         present in the debug info, because some variables dies before
         an `executable_line` instruction is reached.
...
do_smoke(Beam, Node) ->
	...
	{ok,{Mod,[{abstract_code,{raw_abstract_v1,Abstr0}}]}} =
        ...
        %% beam_validator will check for each executable_line instruction
        ...
        %%     such as `rec1`.)
        %%
        %%   * Something is wrong in the mapping from
        %%     `executable_line` instruction to function names,
        %%     causing a variable to be collected into the wrong
        %%     function. (See abstr_executable_lines/1.)
        %%
...
debug_info_vars_2([{Name0,Value}|T], VarAcc, LitAcc) when is_binary(Name0) ->
    ...
    case Value of
        ...
        _ -> debug_info_vars_2(T, VarAcc, [Name|LitAcc])
    end;
debug_info_vars_2([nil,[]|T], VarAcc, LitAcc) ->
...
%% executable_line instructions can be ignored.
%%
...
extract_src_vars({named_fun,_,Name,Cs}, Lc, Acc0) ->
    case any_executable_line_instrs(Cs) of
        false ->
...
extract_src_vars({C,_,Build,Qs0}, Lc, Acc0)
  when C =:= lc; C =:= bc; C =:= mc ->
    case any_executable_line_instrs(Build) of
        false ->
            Qs = extract_sv_qs(Qs0),
            case any_executable_line_instrs(Qs) of
                false ->
                    Acc0;
                true ->
...
extract_cs([{clause,_,Pats,Gs,Body}|Cs], Lc, Acc0) ->
    case Lc andalso not any_executable_line_instrs(Body)  of
        true ->
            extract_cs(Cs, Lc, Acc0);
        false ->
...
extract_body([I], Lc, Acc) ->
    case Lc andalso not any_executable_line_instrs(I) of
        true ->
            Acc;
        false ->
...
%%
%% Return a mapping from executable_line instruction index to function.
%%
abstr_executable_lines(Abstr) ->
    S0 = [{{Name,Arity},abstr_extract_executable_lines(Body)} ||
...
abstr_extract_executable_lines({executable_line,_,Index}, Acc) ->
    [Index|Acc];
abstr_extract_executable_lines([H|T], Acc0) ->
    Acc1 = abstr_extract_executable_lines(H, Acc0),
    abstr_extract_executable_lines(T, Acc1);
abstr_extract_executable_lines(Tuple, Acc0) when is_tuple(Tuple) ->
    abstr_extract_executable_lines(tuple_to_list(Tuple), Acc0);
...
get_debug_info(Mod, Beam) ->
    ...
    0 = Version,
    DebugInfo = decode_debug_info(DebugInfo1, Literals, Atoms, CallOp),
...
decode_debug_info(Code0, Literals, Atoms, CallOp) ->
    case Code0 of
        <<CallOp,Code1/binary>> ->
            ...
            [{FrameSize,List}|decode_debug_info(Code3, Literals, Atoms, CallOp)];
        <<>> ->
            []
    end.
